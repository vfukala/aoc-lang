    ...
    let res = lns.filter(|l| !l.is_empty())
        .filter(|s| {
            ...
            top_ord.iter().fold(HashMap::new(), |mut pmap: HashMap<i32, Vec<Vec<bool>>>, ri| {
                let new_table = if *ri == 8 {
                    let mut tab = vec![vec![false; n + 1]; n + 1];
                    for j in 0..=n {
                        for i in j..=n { 
                            tab[j][i] = (j..=i).any(|k| (tab[j][k] || j == k) && pmap.get(&42).unwrap()[k][i]);
                        }
                    }
                    tab
                } else if *ri == 11 {
                    ...
                    for l in 0..=n {
                        for s in 0..=n - l {
                            for m0 in s..=s + l {
                                for m1 in m0..=s + l {
                                    if (m0 == m1 || tab[m0][m1]) && pmap.get(&42).unwrap()[s][m0] && pmap.get(&31).unwrap()[m1][s + l] {
                                        tab[s][s + l] = true;
                                    }
                                }
                            }
